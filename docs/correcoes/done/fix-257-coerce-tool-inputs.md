...
nota: |
  Coerção: z.coerce.number() em searchGroupsInput (creditMin/creditMax, schemas.ts),
  simulateQuotaInput (creditValue, schemas.ts) e recommendGroupsSchema (creditMin/creditMax/
  budget/desiredTermMonths, ai-sdk.ts — exportado pra teste). Erro barulhento: investigado o
  comportamento real do AI SDK v6 (pnpm store, node_modules/ai/src/generate-text/parse-tool-call.ts)
  — quando o input falha a validação Zod, o SDK emite um chunk `tool-input-error` no fullStream
  (NUNCA chama execute); o loop do runner só tratava "text-delta"/"tool-result"/"tool-call" e
  engolia esse chunk — o único rastro (tool-io-log via onStepFinish) mostrava `output: null`,
  indistinguível de "a tool rodou e não achou nada". Adicionado `case "tool-input-error"` em
  runner.ts + `buildToolInputErrorLogLine`/`logToolInputError` (tool-io-log.ts): log BARULHENTO
  (console.error, outcome=invalid_input, error nomeado) — nunca mais um null mudo. O sentinela
  `__search_needed__` do gap original não existe como construto de código (é o LLM inventando um
  id) — o guard de rebusca (rebuscaDirective/GroupNotInDiscoveryError) já cobre id inexistente;
  a cura de fundo pra esse sintoma é o FIX-258 (rota determinística ANTES da tool-call, próximo
  item deste bloco), não uma mudança aqui.
---
