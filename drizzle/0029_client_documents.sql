-- Documentos do cliente (S3 nosso = fonte da verdade) — FIX-82/83/84, bloco-a.
...
-- ausentes — pendência do bloco-g/FIX-100). migrate usa journal + .sql, então
-- esta migration aplica normalmente via migrate-guard no boot. Ver migration 0028.
...
CREATE INDEX "client_documents_conversation_id_idx" ON "client_documents" USING btree ("conversation_id");--> statement-breakpoint
CREATE INDEX "client_document_downloads_client_document_id_idx" ON "client_document_downloads" USING btree ("client_document_id");
