...
export function CampaignListSection() {
  const { control, register, watch, setValue, trigger, formState } =
    useFormContext();
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Promoções e ofertas que a IA menciona quando o contexto da conversa
          permite. Prioridade alta = mencionada proativamente; baixa = só se o
          cliente perguntar.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-2">
        ...
        >
          {fields.map((field, index) => {
            ...
            const title =
              (watch(`activeCampaigns.${index}.title`) as string) ||
              "Sem título";
            const enabled = watch(
              `activeCampaigns.${index}.enabled`,
            ) as boolean;
                ...
                <AccordionContent>
                  <div className="space-y-3 pb-2">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Textarea
                        rows={3}
                        {...register(`activeCampaigns.${index}.body`)}
                      />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div className="space-y-1.5">
                        ...
                        <DatePicker
                          value={
                            (watch(`activeCampaigns.${index}.startsAt`) as
                              | string
                              | null) ?? null
                          }
                          onChange={(v) =>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <DatePicker
                          value={
                            (watch(`activeCampaigns.${index}.endsAt`) as
                              | string
                              | null) ?? null
                          }
                          onChange={(v) =>
                        ...
                        >
                          <SelectTrigger className="w-full">
                            <SelectValue>
                              {(value) =>
                                PRIORITY_LABEL[
                                  value as keyof typeof PRIORITY_LABEL
                                ] ?? "—"
                              }
                            </SelectValue>
                          </SelectTrigger>
