		...
		documents: [
			{
				...
				files: [{ _id: "6a1f7954cf5174e43aa4a196", documentId: "698ddc4c32efa3125e9d0a4f", sectionId: "698ddc4c32efa3125e9d0a4a", proposalId: "6a1f7953cf5174e43aa4a10a" }],
			},
			{
				name: "RG/CNH - Verso",
				sort: 2,
				...
				sectionId: "698ddc4c32efa3125e9d0a4a",
				files: [{ _id: "6a1f7954cf5174e43aa4a193", documentId: "698ddc4c32efa3125e9d0a50", sectionId: "698ddc4c32efa3125e9d0a4a", proposalId: "6a1f7953cf5174e43aa4a10a" }],
			},
		],
	},
};
...
describe("ConexiaDocsClient — resolveDocumentsToken", () => {
	...
	it("segue o uselink.me (302) e extrai o token do Location", async () => {
		...
		const t = await new ConexiaDocsClient().resolveDocumentsToken("https://www.uselink.me/ZWoibmELt");
...
describe("ConexiaDocsClient — upload", () => {
	...
	beforeEach(() => {
		...
		globalThis.fetch = vi.fn(async (url: string, init: RequestInit = {}) => {
			...
			return { json: async () => ({ success: true, code: 200, message: "Proposta atualizada com sucesso!", data: {} }) } as Response;
		}) as unknown as typeof fetch;
	});
	...
	it("frente → PATCH na URL com section/document/file corretos + multipart", async () => {
		await new ConexiaDocsClient().upload({
			proposalId: "6a1f7953cf5174e43aa4a10a",
			documentsLink: "https://conexia.agxsoftware.com/proposals?documentsToken=6a1f797e10ffff8984dc7201",
		...
		expect(patch?.url).toContain("/section/698ddc4c32efa3125e9d0a4a/document/698ddc4c32efa3125e9d0a4f/client/6a1f7954cf5174e43aa4a196");
	...
	it("verso → escolhe o slot 'Verso' (document _id diferente)", async () => {
		...
		const patch = calls.find((c) => c.init.method === "PATCH");
		expect(patch?.url).toContain("/document/698ddc4c32efa3125e9d0a50/client/6a1f7954cf5174e43aa4a193");
	});
	...
	it("upload falho (success:false) lança — chamador cai pro link", async () => {
		globalThis.fetch = vi.fn(async (_u: string, init: RequestInit = {}) =>
			(init.method ?? "GET") === "GET"
				...
				: ({ json: async () => ({ success: false, code: 500, message: "erro", data: {} }) } as Response),
		) as unknown as typeof fetch;
		await expect(
