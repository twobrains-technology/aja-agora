...
describe("beviOfferToGroupSummary — fixture real imóvel (RODOBENS grupo 2119)", () => {
	...
	it("mapeia os campos centrais da oferta real", () => {
		...
		expect(g.availableSlots).toBe(2); // monthlyAwardedQuotas
	});
});
...
// com a AGX) NÃO é contagem e NUNCA vira availableSlots nem %. O campo
// `contemplationRate` — cópia literal deste valor sob um nome que mentia — foi
// aposentado em 19/08/2026 (PRD, P1).
...
describe("FIX-192 — availableSlots é o monthlyAwardedQuotas real (0 quando ausente); nunca taxaContemplacao", () => {
	...
	it("§7.1 — oferta SEM monthlyAwardedQuotas → availableSlots=0", () => {
	...
	it("taxaContemplacao (fração 0,605) NÃO vira availableSlots (nem 0,605 nem 60,5)", () => {
		...
		expect(g.availableSlots).not.toBe(60.5);
	});
});
