...
describe("FIX-INTEGRIDADE — teto/orçamento fabricado (frase % do teto)", () => {
	it("SYSTEM_PROMPT menciona 'teto DECLARADO pelo próprio usuário' (não default)", () => {
		// Deve haver guardrail que diz: teto é DECLARADO, não inferido/default.
	...
	it("frase de recomendação NÃO menciona 'teto' sem garantia de que foi coletado", () => {
		...
		// Mas isso SÓ aparece SE teto foi declarado.
		// Verificar se há guardrail contra o template ser aplicado quando monthlyBudget=undefined.
		const recommendationSection = SPECIALIST_BASE_PROMPT.match(
			/(## Textos de recomendação[\s\S]*?)(?=^##|$)/m,
		)?.[1] || SYSTEM_PROMPT.match(/(Textos de recomendação[\s\S]*?)(?=\n\n## |$)/i)?.[1] || "";
		// A seção deve mencionar condição: "SEMPRE" ou "apenas quando" o teto foi declarado.
		expect(recommendationSection).toMatch(
			/(apenas quando|se.*orçamento|condicion|blindar)/i,
		);
	});
	...
	it("MOTO NÃO deve emitir 'teto' porque não coleta orçamento mensal", () => {
		...
		const motoSection =
			combined.match(/(MOTO[\s\S]{0,500}(?:orçamento|teto|budget))|moto[\s\S]{0,300}(?:sem.*orçamento|não.*coleta)/i)?.[0] ||
			"";
