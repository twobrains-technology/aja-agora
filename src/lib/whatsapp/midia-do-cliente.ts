...
import { type AnexoDoCliente, relayAvisoAoAtendente, relayUserMediaToAgent } from "./proxy";
...
export async function receberMidiaDoCliente(
	...
	if (!conv) {
		...
		if (!humanoAtende && input.tipo !== "audio") {
	...
	const tipo = tipoDeMidia(media.mimeType) ?? input.tipo;
	...
	// Agente no comando: a foto do RG continua virando slot da proposta. Áudio
	// nunca foi documento de identidade e não entra nesse caminho.
	if (input.tipo !== "audio" && tipo !== "audio") {
