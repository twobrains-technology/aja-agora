...
import { cleanupConversation, closeDb, getConversation } from "../../utils/db";
...
test.describe("EC-07 — Guard de dupla apresentação do card WhatsApp", () => {
	...
	test("CA-25: tool present_whatsapp_optin não emite artifact 2x", async ({ request }) => {
		...
		const hasWhatsappOptinAgain =
			text2.includes('"toolName":"present_whatsapp_optin"') || text2.includes("whatsapp_optin");
