...
const DATABASE_URL = process.env.DATABASE_URL || "postgresql://postgres:postgres@localhost:5435/aja_agora";
...
export async function cleanupConversation(conversationId: string) {
  ...
  try {
    ...
    await db.query("DELETE FROM lead_events WHERE lead_id IN (SELECT id FROM leads WHERE conversation_id = $1)", [
      conversationId,
    ]);
...
export async function getLeadEvents(leadId: string) {
  const db = await getDbClient();
  const result = await db.query("SELECT * FROM lead_events WHERE lead_id = $1 ORDER BY created_at", [leadId]);
  return result.rows;
}
